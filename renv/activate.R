local({
    ...
    renv_bootstrap_download_cran_latest_find <- function(version) {
        ...
        binary <- getOption("renv.bootstrap.binary", default = TRUE) &&
        ...
        for (type in types) {
            for (repos in renv_bootstrap_repos()) {
                ...
                db <- tryCatch(
                    as.data.frame(do.call(utils::available.packages, args), stringsAsFactors = FALSE),
                    error = identity
                )
    ...
    renv_bootstrap_find_cache <- function(version) {
        ...
        file.path(cache, renv_bootstrap_cache_version(), renv_bootstrap_platform_prefix(), "renv", version, md5, "renv")
    }
    ...
    renv_bootstrap_platform_prefix_default <- function() {
        ...
        # expand placeholders
        placeholders <- list(list("%v", format(getRversion()[1, 1:2])), list("%V", format(getRversion()[1, 1:3])))
    ...
    renv_bootstrap_path_absolute <- function(path) {
        substr(path, 1L, 1L) %in%
            c("~", "/", "\\") ||
            (substr(path, 1L, 1L) %in% c(letters, LETTERS) && substr(path, 2L, 3L) %in% c(":/", ":\\"))
    }
