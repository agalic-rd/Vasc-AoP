...
```{r}
apply_model <- function(data, resp_name, family = "Gamma") { # nbinom2("log")
...
make_mod_params_plots <- function(vars, family) {
  purrr::map(
    vars,
    \(var) {
      models <- vasc$data$normal |> 
      ...
      purrr::pwalk(
        ...
        \(var_name, stage, mod) make_signif_boxplot(mod, xaxis = "condition", subtitle = stage) |>
          save_png(stringr::str_glue("mod_{var_name}_vasc_{family}_{stage}"), display = FALSE)
      )
...
gamma_mods <- make_mod_params_plots(gamma_resps, "Gamma")
nbinom_mods <- make_mod_params_plots(negbin_resps, "nbinom2")
```
