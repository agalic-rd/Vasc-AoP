...
vasc_P12 <- list(data = purrr::modify_tree(vasc$data, leaf = \(x) filter(x, stage == "P12")))
```

```{r}
vasc_model <- function(data, resp_name, family = "Gamma") {
    ...
    \(var) {
      models <- data |> 
        ...
        purrr::map_dfr(\(dat) dat |> 
            summarize(stage = first(stage), mod = vasc_model(pick(everything()), var, family), var_name = var) |> 
            select(-any_of(var))
        )
      ...
      purrr::pwalk(
        ...
        \(var_name, stage, mod) make_signif_boxplot(mod, xaxis = "condition", subtitle = stage) |>
          save_png(stringr::str_glue("mod_{var_name}_vasc_{family}_{stage}{if (total_only) '_total' else ''}"), display = FALSE)
      )
      ...
      if (!total_only) {
        purrr::pwalk(
          ...
          \(var_name, stage, mod) make_signif_boxplot_inter(mod, pred1 = "condition", pred2 = "level") |>
            save_png(stringr::str_glue("mod_{var_name}_vasc_{family}_inter_{stage}"), display = FALSE)
        )
      }
...
gamma_mods_total <- make_mod_params_plots(vasc$data$normal, gamma_resps[2:length(gamma_resps)], "Gamma", total_only = TRUE)
