...
```{r}
clearing_model_ds <- function(data, resp_name, family, link) {
  
  form <- stringr::str_glue("{resp_name} ~ condition * level")
  start_values <- list(beta = c(log(mean(data[[resp_name]])), rep(0, 3)))

  if (resp_name %in% c("volume", "network_length", "surface_area", "number_of_segments", "branchpoints", "endpoints")) {
    form <- paste(form, "+ offset(log(cerebellar_volume))")
  }

  form <- paste(form, "+ (1 | mouse)")
...
```{r}
fit_all_models_ds <- function(data, vars, family, link = "log") {
  ...
  purrr::map_dfr(
    ...
    function(var) {
      models <- data |>
        ...
        group_split(stage) |>
        purrr::map_dfr(\(dat) dat |>
            summarize(stage = first(stage), mod = clearing_model_ds(pick(everything()), var, family, link), var_name = var) |>
            select(-any_of(var))
        )
...
gamma_mods_ds <- fit_all_models_ds(clearing_ds, gamma_resps, family = "Gamma", link = "log")
nbinom_mods_ds <- fit_all_models_ds(clearing_ds, negbin_resps, family = "nbinom1", link = "log")

all_mods_ds <- bind_rows(gamma_mods_ds, nbinom_mods_ds)
```

```{r fig.height = 20, fig.width = 15}
fig_1_ds <- c("volume", "network_length", "surface_area")
fig_2_ds <- c("number_of_segments", "branchpoints", "endpoints", "segment_partitioning", "mean_segment_tortuosity")
fig_3_ds <- c("mean_segment_volume", "mean_segment_surface_area", "mean_segment_length", "mean_segment_radius")

all_mods_ds |>
  filter(var_name %in% fig_1_ds) |>
  mutate(var_name = factor(var_name, levels = fig_1_ds)) |> 
  group_by(var_name, stage) |> 
  group_map(
    function(d, g) make_signif_boxplot_inter(first(d$mod), pred1 = "condition", pred2 = "level", stage = first(g$stage), add_interactions = FALSE)
  ) |> 
  patchwork::wrap_plots(ncol = 4, axes = "collect") |>
  save_png("fig_ds_separately_1", subfolder = "IHC", width = 18, height = 15, display = TRUE)

all_mods_ds |>
  filter(var_name %in% fig_2_ds) |>
  mutate(var_name = factor(var_name, levels = fig_2_ds)) |> 
  group_by(var_name, stage) |> 
  group_map(
    function(d, g) make_signif_boxplot_inter(first(d$mod), pred1 = "condition", pred2 = "level", stage = first(g$stage), add_interactions = FALSE)
  ) |> 
  patchwork::wrap_plots(ncol = 4, axes = "collect") |> 
  save_png("fig_ds_separately_2", subfolder = "IHC", width = 18, height = 20, display = TRUE)

all_mods_ds |>
  filter(var_name %in% fig_3_ds) |>
  mutate(var_name = factor(var_name, levels = fig_3_ds)) |> 
  group_by(var_name, stage) |> 
  group_map(
    function(d, g) make_signif_boxplot_inter(first(d$mod), pred1 = "condition", pred2 = "level", stage = first(g$stage), add_interactions = FALSE)
  ) |> 
  patchwork::wrap_plots(ncol = 4, axes = "collect") |> 
  save_png("fig_ds_separately_3", subfolder = "IHC", width = 18, height = 20, display = TRUE)
```
...
make_signif_boxplot_inter(mod_seg_tort_gamma_ds, pred1 = "condition", pred2 = "level", facet = "stage", ncol = 5) |> 
  save_png("mod_seg_tort_gamma_ds_inter", subfolder = "IHC", width = 18, height = 6)
```
...
```{r fig.height = 17, fig.width = 14}
(make_signif_boxplot_inter(mod_vol_gamma_ds, xaxis = "condition", facet = "stage", ncol = 4) /
  (make_signif_boxplot_inter(mod_length_gamma_ds, xaxis = "condition", facet = "stage", ncol = 4) + no_strip) /
  (make_signif_boxplot_inter(mod_area_gamma_ds, xaxis = "condition", facet = "stage", ncol = 4) + no_strip) +
  plot_layout(axes = "collect")) |> 
  save_png("Fig5", subfolder = "IHC", width = 14, height = 17)
```
