...
clearing_model_ds <- function(data, resp_name, family, link) {
  ...
  if (length(unique(data$level)) == 1) {
    form <- stringr::str_glue("{resp_name} ~ condition")
    start_values <- list(beta = c(log(mean(data[[resp_name]])), rep(0, 2)))
  }
...
nbinom_mods_ds <- fit_all_models_ds(clearing_ds, negbin_resps, family = "nbinom1")
...
all_mods_ds |> 
  ...
  group_by(var_name, stage) |> 
  group_map(function(d, g) make_signif_boxplot_inter(first(d$mod), pred1 = "condition", pred2 = "level", stage = first(g$stage))) |> 
  patchwork::wrap_plots(ncol = 4, axes = "collect") |>
