...
clearing_P4 <- list(data = purrr::modify_tree(clearing$data, leaf = \(x) filter(x, stage == "P4")))
clearing_P8 <- list(data = purrr::modify_tree(clearing$data, leaf = \(x) filter(x, stage == "P8")))
clearing_P12 <- list(data = purrr::modify_tree(clearing$data, leaf = \(x) filter(x, stage == "P12")))
clearing_P21 <- list(data = purrr::modify_tree(clearing$data, leaf = \(x) filter(x, stage == "P21")))
```
...
gamma_mods <- make_mod_params_plots(clearing$data$normal, gamma_resps, "Gamma")
nbinom_mods <- make_mod_params_plots(clearing$data$normal, negbin_resps, "nbinom2")

gamma_mods_total <- make_mod_params_plots(clearing$data$normal, gamma_resps, "Gamma", total_only = TRUE)
nbinom_mods_total <- make_mod_params_plots(clearing$data$normal, negbin_resps, "nbinom2", total_only = TRUE)
```
...
gamma_mods <- fit_all_models(clearing$data$normal, gamma_resps, "Gamma", total_only = TRUE)
nbinom_mods <- fit_all_models(clearing$data$normal, negbin_resps, "nbinom2", total_only = TRUE)
...
```{r}
clearing_data_normal_small <- clearing_P4$data$normal |>
...
mod_vol_clearing_gauss <- glmmTMB(
  volume ~ condition * level + offset(log(cerebellar_volume)) + (1 | mouse),
  family = gaussian("log"),
  data = clearing_data_normal_small,
  REML = TRUE
)

cli_h1("[Parameters]")
parameters(mod_vol_clearing_gauss)
cli_h1("[Fitness]")
performance(mod_vol_clearing_gauss)
```

**Gamma:**

```{r}
mod_vol_clearing_gamma <- glmmTMB(
  volume ~ condition * level + offset(log(cerebellar_volume)) + (1 | mouse),
  family = Gamma("log"),
  data = clearing_P4$data$normal,
  REML = TRUE
)

cli_h1("[Parameters]")
parameters(mod_vol_clearing_gamma)
cli_h1("[Fitness]")
performance(mod_vol_clearing_gamma)
```
...
```{r fig.width = 10}
check_model(mod_vol_clearing_gamma)

make_acf_plot(mod_vol_clearing_gamma)
```
...
```{r}
mod_vol_clearing_gamma_dharma <- DHARMa::simulateResiduals(mod_vol_clearing_gamma, plot = FALSE, n = 300, seed = getOption("seed"))
mod_vol_clearing_gamma_dharma_t <- t(mod_vol_clearing_gamma_dharma$simulatedResponse)
```

```{r fig.width = 10}
ppc_plots(mod_vol_clearing_gamma, simulations = mod_vol_clearing_gamma_dharma_t, term = "Condition")
```
...
```{r}
parameters(
  mod_vol_clearing_gamma, exponentiate = should_exp(mod_vol_clearing_gamma),
...
```{r}
car::Anova(mod_vol_clearing_gamma, type = 3)
```
...
distribution_summary(get_data(mod_vol_clearing_gamma), dvs = find_response(mod_vol_clearing_gamma), between = "Condition")

cli_h1("[Emmeans]")

emmeans(mod_vol_clearing_gamma, specs = "Condition", type = "response")
```
...
make_signif_boxplot(mod_vol_clearing_gamma, xaxis = "condition", subtitle = "P4") |>
  save_png("mod_vol_clearing_gamma_P4")
```

```{r fig.width = 6, fig.height = 5}
make_signif_boxplot_inter(mod_vol_clearing_gamma, pred1 = "condition", pred2 = "level") |> 
  save_png("mod_vol_clearing_gamma_inter_P4")
```
