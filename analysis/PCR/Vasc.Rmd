...
(vasc_data$models <- vasc_data$clean
  ...
  |> map_dfr(
    \(d) summarize(d, mod = pick(condition, dcq) |> ND_model() |> list(), .by = c(stage, gene)), 
...
(vasc_data$predictions <- vasc_data$models
  ...
  |> select(stage, gene, fold, expression, matches("-|/"), LCB, UCB, p.value)
)
```
...
(
  ...
  |> filter(p.value <= .05)
  |> select(stage, gene, fold, p.value, expression, pathway, effect)
